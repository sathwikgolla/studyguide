...
export default function SheetCard({ sheet, sheetIndex, progressKey, filter, open: defaultOpen }) {
  const [open, setOpen] = useState(defaultOpen)
  const { progressSummary } = useProgress()
  const stats = progressSummary.perSheet[progressKey] ?? {
    done: 0,
    total: sheet.questions?.length ?? 0,
    pct: 0,
  }
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-4">
            <div className="min-w-0">
              <h3 className="truncate text-base font-semibold text-white">{sheet.title}</h3>
              <p className="text-xs text-neutral-500">
                {sheet.category} · {sheet.questions?.length ?? 0} questions
              </p>
            </div>
